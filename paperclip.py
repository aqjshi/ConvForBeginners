...
# Load ONNX model
core = Core()
model_onnx = core.read_model("resnet_gaussian.onnx")

# Compile model for desired device (CPU or GPU)
compiled_model = core.compile_model(model_onnx, "AUTO")  # or "CPU", "GPU"
...
with torch.no_grad():
    for images, ids in test_bar:
        ...
        # Run inference
        results = infer_request.infer({compiled_model.input(0): images_np})
        
        mean = results[compiled_model.output("mean")]
        std = results[compiled_model.output("std")]
